  ...
  return (
    <>
      <button
        onClick={refresh}
        disabled={busy}
        className="flex w-full items-center justify-center gap-2 rounded-lg border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs font-medium text-slate-300 transition hover:border-white/20 hover:bg-white/[0.08] hover:text-white active:scale-[0.99] disabled:opacity-60"
      >
        <RefreshCw className={`size-3.5 text-cyan-400 ${busy ? 'animate-spin' : ''}`} />
