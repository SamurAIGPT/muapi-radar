...
export async function detectNarratives(projectId: number): Promise<number> {
  ...
  const payload = posts.map((p) => ({
    id: p.id, author: p.author, source: p.source, text: p.content.slice(0, 220),
  }));
  const text = await callClaude(MODELS.sonnet, 'narratives', SYSTEM, JSON.stringify(payload), 2500, true);
  ...
  for (const g of groups) {
    ...
    await db.insert(narratives).values({
      ...
      stance: ['positive', 'negative', 'neutral', 'polarizing'].includes(g.stance) ? g.stance : 'neutral',
