...
export const tiktok: Connector = {
  ...
  label: 'TikTok',
  tier: 'premium',
  enabled: () => isMuapiConfigured() || Boolean(cfg('TIKTOK_CLIENT_KEY') && cfg('TIKTOK_CLIENT_SECRET')),
  disabledReason: 'Requires Muapi API Key or TikTok Research API credentials in Settings',
  async fetchMentions(q) {
    const query = q.anyTerms.slice(0, 3).join(' OR ');
    if (!query) return [];

    // 1. Muapi social.search_posts with platform=tiktok
