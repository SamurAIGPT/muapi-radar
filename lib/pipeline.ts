  ...
  try {
    ...
    for (const project of allProjects) {
      ...
      const theme = [project.name, project.semanticContext, `termini: ${project.keywords.join(', ')}`]
        .filter(Boolean).join(' — ');
      const analysis = aiOn
        ? await analyzePendingMentions(project.id, theme).catch((e) => {
            console.warn(`[pipeline] analisi fallita per "${project.name}":`, e);
      ...
      const newAlerts = await detectAlerts(project.id, { aiContext: aiOn }).catch((e) => {
        console.warn(`[pipeline] alerts falliti per "${project.name}":`, e);
        return 0;
      });
      const trendCount = await computeTrends(project.id).catch((e) => {
        console.warn(`[pipeline] trends falliti per "${project.name}":`, e);
        return 0;
      });
      ...
      if (newAlerts > 0) {
        ...
        await notifyAlerts(project.name, fresh.map((f) => ({
          ...
          message: (f.data as { explanation?: string } | null)?.explanation
            ...
            : f.message,
        }))).catch((e) => console.warn('[pipeline] notifyAlerts error:', e));
      }

      if (opts.full && aiOn) {
        row.timeline = await extractTimelineEvents(project.id).catch(() => 0);
        row.trendsExplained = await explainTrends(project.id).catch(() => 0);
        row.narratives = await detectNarratives(project.id).catch(() => 0);
        row.stories = await clusterNewsStories(project.id).catch(() => 0);
        row.rated = await scoreTopContent(project.id, project.name).catch(() => 0);
        const briefData = await collectBriefData(project.id).catch(() => null);
        if (briefData) {
          row.brief = await generateDailyBrief(project.id, project.name, briefData).catch(() => null);
        }
        ...
        row.pov = await refreshPointOfViewIfStale(project.id).catch(() => null);
