...
export function CommandHint({ locale = 'en' }: { locale?: Locale }) {
  ...
  return (
    <button
      ...
      className="flex w-full items-center gap-2 rounded-lg border border-white/[0.07] bg-white/[0.02] px-2.5 py-1.5 text-left text-xs text-slate-400 transition hover:border-white/15 hover:bg-white/[0.05] hover:text-slate-200">
      <Search className="size-3.5 shrink-0 text-slate-500" />
      <span className="flex-1 truncate">{t('nav.searchPlaceholder', 'Go to…')}</span>
      <kbd className="shrink-0 rounded border border-white/10 bg-white/[0.04] px-1.5 py-0.5 font-mono text-[10px] text-slate-400">
        {mac ? '⌘' : 'Ctrl'}K
      </kbd>
    </button>
  );
